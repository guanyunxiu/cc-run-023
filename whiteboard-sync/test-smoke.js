...
async function main() {
  ...
  console.log('\n[7] 版本恢复：补偿信封经服务端广播，两个在线客户端 + 晚加入者都收敛到版本');
  const R = require('./public/replay.js');
  const vroom = 'ver-' + Date.now();
  const P = makeClient('P', vroom), Q = makeClient('Q', vroom);
  await P.connect(); await Q.connect();
  // 版本内容：keep 矩形
  P.issue({ kind: 'create', objects: [{ oid: 'keep', type: 'rect',
    fields: { x: 10, y: 10, w: 20, h: 20, z: '1' } }] });
  await sleep(200);
  const versionSnap = P.doc.snapshot(P.clock.vc);   // 在此刻保存版本
  // 版本之后继续演化：移动 keep、新建 later
  P.issue({ kind: 'set', oid: 'keep', fields: { x: 999 }, prev: { x: 10 } });
  await sleep(100);
  Q.issue({ kind: 'create', objects: [{ oid: 'later', type: 'ellipse',
    fields: { x: 0, y: 0, w: 5, h: 5, z: '2' } }] });
  await sleep(250);
  assert(P.doc.get('keep').x === 999 && P.doc.get('later'), 'fixture: diverged from saved version');

  // P 端恢复版本：生成补偿信封（delete later + set keep.x=10），普通 ops 提交
  const restoreEnvs = R.restoreEnvelopes(P.clock, P.doc, versionSnap);
  const kinds = restoreEnvs.map((e) => e.op.kind).sort();
  assert(kinds.includes('delete') && kinds.includes('set'),
    `restore builds delete+set compensations (${kinds.join(',')})`);
  P.issueBatch(restoreEnvs);
  await sleep(300);
  const sig = (d) => d.liveObjects().map((o) => o.oid + '@' + (o.x || '')).sort().join('|');
  assert(sig(P.doc) === 'keep@10', `P converged to version (${sig(P.doc)})`);
  assert(sig(Q.doc) === 'keep@10', `Q converged to version via broadcast (${sig(Q.doc)})`);

  // 晚加入者从服务端快照也得到恢复后的内容
  const L = makeClient('L', vroom); await L.connect();
  assert(sig(L.doc) === 'keep@10', `late joiner sees restored version state (${sig(L.doc)})`);
  L.close(); P.close(); Q.close();
