...
async function main() {
  ...
  assert(Array.isArray(ack.seqs) && ack.seqs.length === ack.ids.length &&
    ack.seqs[ack.ids.indexOf(env.id)] === ack.lastSeq,
    `ACK carries per-id authoritative seqs (seqs=${JSON.stringify(ack.seqs)})`);
  await sleep(150);
